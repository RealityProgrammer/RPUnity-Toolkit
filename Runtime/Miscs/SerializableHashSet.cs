...
// https://referencesource.microsoft.com/#system.core/system/Collections/Generic/HashSet.cs,06d905a0ac76e4f5
namespace RealityProgrammer.UnityToolkit.Core.Miscs {
    [Serializable]
    public class SerializableHashSet<T> : ICollection<T>, ISet<T>, IReadOnlyCollection<T> {
...
#endif
        [SerializeField] private int[] m_buckets;
        [SerializeField] private Slot[] m_slots;
        [SerializeField] private int m_count;
        [SerializeField] private int m_lastIndex;
        [SerializeField] private int m_freeList;
        private IEqualityComparer<T> m_comparer;
        [SerializeField] private int m_version;

        [SerializeField] private T _candidateValue;
        ...
        internal bool ContainsCandidate() {
            return Contains(_candidateValue);
        }
        ...
        internal List<object> GetValueLookupList() {
        ...
        internal List<int> GetSlotHashCodes() {
            List<int> ret = new List<int>(Count);
            ...
            while (index < m_lastIndex) {
                if (m_slots[index].hashCode >= 0) {
                    ret.Add(m_slots[index].hashCode);
                    index++;
                    continue;
                }
        ...
        public SerializableHashSet() : this(EqualityComparer<T>.Default) {
        }
        ...
        public SerializableHashSet(IEnumerable<T> collection, IEqualityComparer<T> comparer) : this(comparer) {
            if (collection == null) {
                throw new ArgumentNullException("collection");
            }

            var otherAsHashSet = collection as SerializableHashSet<T>;
        ...
        // Initializes the SerializableHashSet from another SerializableHashSet with the same element type and
        // equality comparer.
        private void CopyFrom(SerializableHashSet<T> source) {
            ...
            int threshold = PrimeHelper.ExpandPrime(count + 1);
        ...
        public bool Contains(T item) {
            if (m_buckets != null) {
                ...
                // see note at "SerializableHashSet" level describing why "- 1" appears in for loop
        ...
        /// <returns>true if removed; false if not (i.e. if the item wasn't in the SerializableHashSet)</returns>
        ...
        #region SerializableHashSet methods
        /// <summary>
        /// Add item to this SerializableHashSet. Returns bool indicating whether item was added (won't be 
        ...
        /// <summary>
        /// Take the union of this SerializableHashSet with other. Modifies this set.
        /// 
        ...
        public void IntersectWith(IEnumerable<T> other) {
            ...
            if (otherAsCollection != null) {
                ...
                SerializableHashSet<T> otherAsSet = other as SerializableHashSet<T>;
        ...
        public void SymmetricExceptWith(IEnumerable<T> other) {
            ...
            SerializableHashSet<T> otherAsSet = other as SerializableHashSet<T>;
            // If other is a SerializableHashSet, it has unique elements according to its equality comparer,
        ...
        public void TrimExcess() {
            ...
            if (m_count == 0) {
                ...
                int newSize = PrimeHelper.GetPrime(m_count);
        ...
        private void IncreaseCapacity() {
            ...
            int newSize = PrimeHelper.ExpandPrime(m_count);
            if (newSize <= m_count) {
                throw new ArgumentException("Attempt to increase set capacity, result in capacity overflow");
            }
        ...
        /// <summary>
        /// Adds value to SerializableHashSet if not contained already
        ...
        // when constructing from another SerializableHashSet.
        ...
        /// <param name="other"></param>
        /// <returns></returns>
        private bool IsSubsetOfHashSetWithSameEC(SerializableHashSet<T> other) {
        ...
        /// </summary>
        /// <param name="other"></param>
        private void IntersectWithHashSetWithSameEC(SerializableHashSet<T> other) {
            for (int i = 0; i < m_lastIndex; i++) {
                if (m_slots[i].hashCode >= 0) {
        ...
        /// </summary>
        /// <param name="other"></param>
        private void SymmetricExceptWithUniqueHashSet(SerializableHashSet<T> other) {
            foreach (T item in other) {
                if (!Remove(item)) {
                    AddIfNotPresent(item);
                }
        ...
        /// Implementation notes:
        /// 
        /// Used for symmetric except when other isn't a SerializableHashSet. This is more tedious because 
        /// other may contain duplicates. SerializableHashSet technique could fail in these situations:
        /// 1. Other has a duplicate that's not in this: SerializableHashSet technique would add then 
        /// remove it.
        /// 2. Other has a duplicate that's in this: SerializableHashSet technique would remove then add it
        /// back.
        ...
        /// is only used when other is an IEnumerable and not a SerializableHashSet. If other is a SerializableHashSet
        ...
        /// <param name="comparer"></param>
        /// <returns></returns>
        internal static bool HashSetEquals(SerializableHashSet<T> set1, SerializableHashSet<T> set2, IEqualityComparer<T> comparer) {
        ...
        /// <param name="set2"></param>
        /// <returns></returns>
        private static bool AreEqualityComparersEqual(SerializableHashSet<T> set1, SerializableHashSet<T> set2) {
        ...
        internal struct Slot {
            [SerializeField] internal int hashCode;      // Lower 31 bits of hash code, -1 if unused
            [SerializeField] internal int next;          // Index of next entry, -1 if last
        ...
        public struct Enumerator : IEnumerator<T>, System.Collections.IEnumerator {
            private SerializableHashSet<T> set;
            private int index;
            private int version;
            private T current;

            internal Enumerator(SerializableHashSet<T> set) {
                this.set = set;
                index = 0;
